...
for (k in 1:10){
  set.seed(k)
  rand_order2<-sample(nrow(ELO_genes2))
  ELO_genes2$ELO_hrs_RANDOM<-ELO_genes2$ELO_hrs[rand_order2]
...
write.table(TotalR2,"R2_shuffleddata_ELO_full.txt",quote = F,sep = "\t")
